...
原有的 44 个阻断项不是 44 个独立回归错误，而是功能矩阵中所有尚未达到 `pass` 的必需功能。第二批迁移完成后阻断数降为 36；第三批三个阶段依次降为 33、32 和 29；第四批降为24；第五批把关系与治理事务中的五项闭环后降为19；第六批完成战斗聚合层后降为16。迄今转为完整通过的功能包括：
...
当前矩阵为 53 项：37 项 `pass`、8 项 `partial`、8 项 `missing`。101 个冻结的 V1 公共操作仍被完整且唯一归档，矩阵结构无错误。V2 仍不可正式切换。
...
| 战斗、战争与魔道聚合 | 3 | `demonic.prison`、`demonic.captives`、`demonic.puppets_souls` |
...
第六批最终全仓结果为 `638 passed`；矩阵审计为101/101入口、0个结构错误、16个已分类阻断项、0个影子执行错误。既有鬼修共享随机流差异仍为唯一影子告警。
...
6. 队伍、地形、阵法和战争已经共享战斗快照；下一批迁移监禁、俘虏操作、傀儡与魂魄的魔道聚合。
7. 最后迁移 DLC 深层状态机、正式 HTTP/前端，并把影子适配器扩展到全部 101 个操作。

每一项只能在命令、权威状态、跨领域事件、失败原子性、不变量、存档迁移和具体测试节点全部存在后改为 `pass`。在 16 项真正清零前，Step 10 继续保持禁止状态。
