...
原有的 44 个阻断项不是 44 个独立回归错误，而是功能矩阵中所有尚未达到 `pass` 的必需功能。第二批迁移完成后阻断数降为 36；第三批三个阶段依次降为 33、32 和 29；第四批降为24；第五批把关系与治理事务中的五项闭环后降为19；第六批完成战斗聚合层后降为16；第七批完成魔道聚合后降为11。迄今转为完整通过的功能包括：
...
当前矩阵为 53 项：42 项 `pass`、6 项 `partial`、5 项 `missing`。101 个冻结的 V1 公共操作仍被完整且唯一归档，矩阵结构无错误。V2 仍不可正式切换。
...
第七批将V2存档升级到Schema 15，并新增监禁、俘虏、傀儡、魂魄、夺舍、黑市傀儡交易及旧档导入的端到端验证。全仓结果为 `645 passed`；矩阵审计为101/101入口、0个结构错误、11个已分类阻断项、0个影子执行错误。既有鬼修共享随机流差异仍为唯一影子告警。
...
当前V2共加载363个基础及DLC剧情定义。效果注册表已迁移26种效果，经过后续事件链闭包检查后有194个事件可安全进入随机池；关系生擒、侍妾求娶/拒绝报复、宗门加入、祖师复权与战争先锋效果已经接入，其他未迁移的关系制裁或DLC专属效果仍明确隔离。因此`story.interactive_events`仍为`partial`，尚不能标记为`pass`。
...
7. 下一批迁移 DLC 深层状态机、正式 HTTP/前端，并继续扩展影子适配器。

每一项只能在命令、权威状态、跨领域事件、失败原子性、不变量、存档迁移和具体测试节点全部存在后改为 `pass`。在 11 项真正清零前，Step 10 继续保持禁止状态。
