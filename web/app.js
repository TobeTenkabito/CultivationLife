...
function renderNatalArtifact(system) {
  ...
  const refineTools=document.createElement('div');refineTools.className='natal-refine-tools';
  const refine=document.createElement('button');refine.className='natal-action';refine.textContent=system.level>=system.max_level?'祭炼圆满':`灵石温养 · ${number(system.refine_cost)}`;refine.dataset.natalUnavailable=system.can_refine?'0':'1';refine.onclick=()=>mutate(`/api/games/${game.id}/natal-artifact`,{action:'refine'});
  const refineAll=document.createElement('button');refineAll.className='natal-action natal-refine-all';refineAll.textContent=system.level>=system.max_level?'一键温养 · 已圆满':`一键温养 ×${number(system.refine_all_count||0)} · ${number(system.refine_all_cost||0)}`;refineAll.dataset.natalUnavailable=system.can_refine_all?'0':'1';refineAll.title='连续消耗灵石温养，直到灵石不足或本命法宝祭炼圆满';refineAll.onclick=()=>mutate(`/api/games/${game.id}/natal-artifact`,{action:'refine_all'});refineTools.append(refine,refineAll);head.append(identity,refineTools);root.appendChild(head);
...
function renderSageSystem(system) {
  ...
  if (current) {
    ...
    ['sage_preach','sage_teach','sage_answer'].forEach(action => {
  ...
  doctrines.forEach(doctrine => {
    ...
    (doctrine.members || []).forEach(member => {
      ...
      if (!member.is_player && system.membership_id) {
        ...
        memberRow.appendChild(debate);
      }
      ranking.appendChild(memberRow);
    });
...
function renderButtons() {
  ...
  const bodyTrain = $('#body-train-action');
  if (bodyTrain) bodyTrain.disabled = busy || !game?.player?.alive || !!game?.pending_event || !!game?.imprisonment || (!!guixuSession && !trappedInGuixu) || (!trappedInGuixu && (!game?.body_cultivation?.technique || game?.body_cultivation?.ready || game?.body_cultivation?.layer >= game?.body_cultivation?.max_layer));
  const senseTrain = $('#sense-train-action');
  if (senseTrain) senseTrain.disabled = busy || !game?.player?.alive || !!game?.pending_event || !!game?.imprisonment || (!!guixuSession && !trappedInGuixu) || (!trappedInGuixu && !game?.player?.divine_sense?.technique);
  const senseBreakthrough = $('#sense-breakthrough-action');
