...
const GROUPS={
  cultivation:['修行','境界、功法、炼体、神识与变化之术'],world:['天地','地图旅行、跨界、飞升与全局设置'],
  relationships:['因果','道侣、道友、师徒、侍妾、家族与队伍'],faction:['势力','宗门治理、外交、内政、战争与悬赏'],
  economy:['营生','坊市、拍卖、黑市与物品交易'],production:['百艺','灵田、炼器、阵法与本命法宝'],
  combat:['征伐','战斗、监狱、傀儡、炼魂与夺舍'],dlc:['异道','鬼修、妖修与天庭扩展系统'],
};
...
const OPERATIONS=[
  ...
  op('ascend-world','world','选择飞升世界','使用 V2 飞升流程，可邀请境界合适的道侣或道友同行。',[textField('destination_world_id','目标世界','worlds',{required:true}),multiField('invited_ids','邀请同行','ascensionInvites')],{danger:true}),
  ...
  op('concubine-status','relationships','侍妾身份','处理自己受高阶修士约束的侍妾身份。',[selectField('action','操作',[choice('escape','谋求脱身'),choice('depend','主动依附'),choice('request_technique','索要功法'),choice('request_stones','索要灵石'),choice('request_equipment','索要装备')]),textField('method','脱身方式（仅脱身时）')]),
  op('relationship-capture','relationships','关系事件','尝试通过事件建立一种人物关系。',[selectField('kind','关系类型',RELATION_KINDS),textField('target_id','对象','npcs',{required:true})]),
  ...
  op('faction-dispatch','faction','派遣门人','派遣宗门成员执行任务。',[textField('target','派遣目标','factionMembers',{required:true})]),
  ...
  op('vassal-transfer','faction','附庸人员调动','在宗门、家族或附庸势力间调动人员。',[selectField('kind','势力类型',[choice('sect','宗门'),choice('family','家族'),choice('race','族群')]),textField('target_id','目标势力','factions',{required:true}),textField('npc_id','人物','npcs',{required:true})]),
  ...
  op('auction-identity','economy','拍卖会身份','选择在本场拍卖会使用的化名。',[textField('alias','化名','',{required:true,maxlength:20})]),
  ...
  op('formation-ground-deploy','production','部署护山阵','为玩家、宗门或家族部署地面阵法。',[selectField('owner_kind','归属',[choice('player','个人'),choice('sect','宗门'),choice('family','家族')])]),
  ...
  op('fight','combat','发起战斗','选择目标与战斗目的。',[textField('target_id','对手','npcs',{required:true}),selectField('objective','目的',[choice('duel','切磋'),choice('kill','死斗'),choice('capture','擒获'),choice('plunder','掠夺')])],{danger:true}),
  ...
  op('heavenly-election','dlc','天庭选举','决定投票、结盟或支持某项承诺。',[selectField('method','方式',[choice('none','弃权'),choice('vote','投票'),choice('pledge','结盟')]),textField('pledge_id','承诺或候选','heavenlyPledges')]),
];
...
function rowOption(row,fallback='id'){
  ...
  const value=row.value??row.id??row[`${fallback}_id`]??row.definition_id??row.item_id??row.npc_id??row.name;
...
function operationAvailable(spec){if(spec.group==='dlc'){if(spec.operation.startsWith('ghost-'))return at('ghost_system.available')!==false;if(spec.operation.startsWith('monster-')||spec.operation.startsWith('custom-lineage'))return at('monster_system.available')!==false;if(spec.operation.startsWith('heavenly-'))return at('heavenly_court.visible')!==false;}return true;}
function renderOperations(){
  ...
  $('#operation-title').textContent=query?'搜索结果':group[0];$('#operation-summary').textContent=query?`找到 ${rows.length} 个正式入口`:`${group[1]} · ${rows.length} 项`;
  $('#operation-list').innerHTML=rows.length?rows.map(spec=>{const available=operationAvailable(spec);return`<details class="operation-card" data-operation-card="${esc(spec.operation)}"><summary><span><strong>${esc(spec.label)}</strong><small>${esc(spec.description)}</small></span><span class="route-badge">${available?'可操作':'未解锁'}</span></summary><form data-operation="${esc(spec.operation)}" class="operation-form">${spec.fields.map(field=>renderField(field,spec.operation)).join('')}<div class="submit-row"><span class="route-name">${esc(spec.operation)}</span><button type="submit" class="${spec.danger?'danger':'primary'}"${available?'':' disabled'}>${spec.danger?'确认执行':'执行'}</button></div></form></details>`;}).join(''):'<p class="empty-note">没有找到相符功能。</p>';
...
async function submitOperation(event){event.preventDefault();const form=event.currentTarget,spec=OPERATIONS.find(row=>row.operation===form.dataset.operation);if(!form.reportValidity())return;if(spec?.danger&&!window.confirm(`确定执行“${spec.label}”吗？此操作可能改变重要状态。`))return;await command(form.dataset.operation,collectPayload(form));}
async function command(operation,payload={}){
  ...
  try{const result=await api(`/api/v2/games/${game.id}/${operation}`,{method:'POST',body:JSON.stringify(payload)}),next=result.game||result;game=next?.player?next:await api(`/api/v2/games/${game.id}`);render();toast(result.events?.at?.(-1)?.message||'命令完成，存档已更新','success');}
...
function renderSaves(){const node=$('#saves');node.innerHTML=(window.saves||[]).length?window.saves.map(row=>`<button data-id="${esc(row.game_id||row.id)}">续接 · ${esc(row.player_name||row.name)}</button>`).join(''):'<span class="muted">尚无 V2 存档</span>';node.querySelectorAll('button').forEach(button=>button.onclick=()=>load(button.dataset.id));}
function renderLegacySaves(){const rows=window.legacySaves||[];$('#legacy-box').classList.toggle('hidden',!rows.length);$('#legacy-saves').innerHTML=rows.map(row=>`<button data-file="${esc(row.file_name)}">导入 · ${esc(row.name)}（${esc(row.game_version)}）</button>`).join('');$('#legacy-saves').querySelectorAll('button').forEach(button=>button.onclick=()=>importLegacy(button.dataset.file));}
async function importLegacy(fileName){if(busy)return;busy=true;try{const result=await api('/api/v2/import',{method:'POST',body:JSON.stringify({file_name:fileName})});game=result.game;window.saves=(await api('/api/v2/games')).games;renderSaves();render();toast('旧版存档已备份并导入','success');}catch(error){toast(error.message,'error');}finally{busy=false;}}
async function load(id){game=await api(`/api/v2/games/${id}`);render();}
function renderNav(){$('#system-nav').innerHTML=Object.entries(GROUPS).map(([id,[label]])=>`<button type="button" data-group="${id}" class="${id===activeGroup?'active':''}">${esc(label)}<span>${OPERATIONS.filter(row=>row.group===id).length}</span></button>`).join('');$('#system-nav').querySelectorAll('button').forEach(button=>button.onclick=()=>{activeGroup=button.dataset.group;$('#operation-search').value='';renderNav();renderOperations();});}
...
function renderStatus(){const flags=[['存活',game.player.alive!==false],['事件待决',Boolean(game.pending_event)],['服刑',Boolean(at('demonic_system.imprisonment.active'))],['宗门',Boolean(game.faction?.exists||game.faction?.id)],['家族',Boolean(game.family?.exists)],['拍卖会',['scheduled','open','black_market'].includes(game.auction?.status)]];$('#status-flags').innerHTML=flags.map(([label,on])=>`<span class="pill ${on?'good':''}">${esc(label)} · ${on?'是':'否'}</span>`).join('');}
function render(){
  $('#start').classList.add('hidden');$('#game').classList.remove('hidden');$('#name').textContent=game.player.name;const c=game.player.cultivation;$('#realm').textContent=`${c.realm_name} · ${c.layer}层 · 纪年 ${game.clock.year}`;
  $('#stats').innerHTML=[['年龄',game.player.age],['寿元',game.player.lifespan??'不朽'],['机缘',`${Math.round(c.opportunity)}/${c.opportunity_required}`],['心魔',Math.round(c.heart_demon)],['世界',game.world.world_name||game.world.name],['地点',game.world.location_name||game.world.location?.name||'未知']].map(([key,value])=>`<div><dt>${esc(key)}</dt><dd>${esc(value)}</dd></div>`).join('');
  ...
  renderNav();renderStatus();renderOperations();renderViews();$('#save-state').textContent='已保存';
}
async function boot(){[config,{games:window.saves},{games:window.legacySaves}]=await Promise.all([api('/api/v2/config'),api('/api/v2/games'),api('/api/v2/legacy-saves')]);fill('#roots',config.roots);fill('#paths',config.paths);renderSaves();renderLegacySaves();}
$('#operation-search').oninput=renderOperations;$('#action-units').onchange=()=>game&&render();$('#new').onclick=()=>{$('#game').classList.add('hidden');$('#start').classList.remove('hidden');};
$('#create').onsubmit=async event=>{event.preventDefault();const payload=Object.fromEntries(new FormData(event.target));if(!payload.seed)delete payload.seed;else payload.seed=Number(payload.seed);try{game=await api('/api/v2/games',{method:'POST',body:JSON.stringify(payload)});render();}catch(error){toast(error.message,'error');}};
