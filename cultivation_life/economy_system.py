...
class EconomySystemMixin:
    ...
    def _ensure_market(self, game: GameState, rng: Any) -> bool:
        ...
        for index in range(int(MARKET_SETTINGS["offer_count"])):
            seed_pool = [
            ...
            pool = [entry for entry in world_goods if entry["tier"] == offer_tier]
            if not pool:
                pool = [entry for entry in world_goods if entry["tier"] == tier]
                offer_tier = tier
                rare_next_tier = False
            if not pool:
                break
