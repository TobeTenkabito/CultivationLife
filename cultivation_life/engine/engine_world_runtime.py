...
def _ensure_sects(deps: WorldRuntimeDependencies, game: GameState) -> None:
    fresh = deps._new_sects()
    ...
    for sect_id, new_sect in fresh.items():
        ...
        for npc in game.sects[sect_id].npcs:
            ...
            if not npc.spirit_root:
                npc.spirit_root = (
                    ...
                    else deps._random_npc_root(npc.realm_index, random.Random(f"{game.seed}:{npc.id}"))
                )
            if game.world_rules_version < 4 and template:
                npc.path = template.path
            elif not npc.path or npc.path not in PATH_NAMES:
                npc.path = template.path if template else deps._random_npc_path(sect_id, random.Random(f"path:{game.seed}:{npc.id}"))
            ...
            if not npc.treasure_item_id and not npc.treasure_looted:
                npc.treasure_item_id = deps._select_npc_treasure(
        ...
        deps._compact_sect_roster(game, game.sects[sect_id])
    game.world_rules_version = max(game.world_rules_version, 7)


def _compact_sect_roster(deps: WorldRuntimeDependencies, game: GameState, sect: SectState) -> bool:
...
def _ensure_world_npcs(deps: WorldRuntimeDependencies, game: GameState) -> bool:
    ...
    for npc_id, npc in fresh.items():
        ...
        if not current.treasure_item_id and not current.treasure_looted:
            current.treasure_item_id = deps._select_npc_treasure(
...
def _enforce_world_realm_caps(deps: WorldRuntimeDependencies, game: GameState) -> bool:
    ...
    demon_cap = deps._world_realm_cap("demon")
...
def _migrate_true_demon_races(deps: WorldRuntimeDependencies, game: GameState) -> bool:
