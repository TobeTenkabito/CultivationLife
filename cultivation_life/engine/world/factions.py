...
from ..dependencies import FactionDependencies
...
def _has_race_voice(deps: FactionDependencies, game: GameState) -> bool:
    if deps._intrigue_enabled():
        race_id = deps._player_allegiance_race(game.player)
        return deps._world_supports(game.player.world, "races") and deps._intrigue_has_decision_authority(game, "race", race_id)
    realm_index, _ = deps._actual_player_realm(game.player)
    required = int(WORLD_SYSTEMS["world_travel"]["required_realm"])
    return deps._world_supports(game.player.world, "races") and deps._player_allegiance_race(game.player) == "human" and realm_index >= required
...
def _has_sect_voice(deps: FactionDependencies, game: GameState) -> bool:
    ...
    return bool(
        ...
        and (sect.founded_by_player or realm_index >= deps._governance_threshold(player.world))
    )


def _has_family_voice(deps: FactionDependencies, game: GameState) -> bool:
    family = game.family
    if deps._intrigue_enabled():
        return bool(
            family and family.world == game.player.world
            and deps._intrigue_has_decision_authority(game, "family", family.id)
        )
    return bool(
...
def _dissolve_player_sect(deps: FactionDependencies, game: GameState, sect: SectState, reason: str) -> None:
    """Disband a weak player sect without falsely killing every former member."""
    sect.extinct = True
    for npc in deps._sect_members(game, sect):
        if npc.alive:
            npc.faction_id = None
...
def _maybe_founded_sect_pressure(deps: FactionDependencies, game: GameState, rng: random.Random) -> bool:
    ...
    if not sect or sect.extinct or not sect.founded_by_player or sect.world != player.world:
        return False
    actual_realm, _ = deps._actual_player_realm(player)
    governance_threshold = deps._governance_threshold(player.world)
    qualified_members = [
        npc for npc in deps._sect_members(game, sect)
    ...
    game.pending_event = deps._instantiate_event(deps.events_by_id[event_id], game, rng)
    ...
    game.pending_event["_history_tags"] = [
        tag for tag in deps.events_by_id[event_id].get("tags", []) if not tag.startswith("world:")
    ] + [f"world:{player.world}"]
    game.pending_event["body"] += f"（正面守山建议战力 {required_power:.0f}；此前护山失败 {sect.pressure}/3 次。）"
    return True


def _annual_sect_update(deps: FactionDependencies, game: GameState, rng: random.Random) -> list[str]:
    news: list[str] = []
    player = game.player
    ...
    for sect_id, sect in game.sects.items():
        ...
        for npc in sect.npcs:
            ...
            if npc.wounds > 0 and rng.random() < 0.35:
                npc.wounds -= 1
            tribulation = deps._resolve_npc_periodic_tribulation(game, npc, rng, sect.name)
            if tribulation:
        ...
        if (
            ...
            if newcomer.realm_index >= 3:
                ...
                game.history.append(HistoryRecord(
                    ...
                    f"{newcomer.name}以{deps._npc_realm_name(newcomer)}修为加入{sect.name}，列为{newcomer.title}。",
    ...
    news.extend(deps._annual_offspring_and_family_update(game, rng))
    deps._annual_relationship_update(game, rng)
    faction_meta = deps._faction_meta(game, player.faction_id or "")
    if (
...
def _annual_race_diplomacy_update(deps: FactionDependencies, game: GameState, rng: random.Random) -> list[str]:
    """Compatibility wrapper. Diplomacy now advances once per action unit, not once per year."""
    return deps._advance_diplomacy_unit(game, rng)


def _advance_diplomacy_unit(deps: FactionDependencies, game: GameState, rng: random.Random) -> list[str]:
    ...
    for kind, relations in (("race", game.race_relations), ("sect", game.sect_relations)):
        for key, relation in relations.items():
            ...
            first_name, second_name = deps._power_name(game, kind, first), deps._power_name(game, kind, second)
            world = game.player.world if kind == "race" and deps._world_supports(game.player.world, "races") else "spirit" if kind == "race" else game.sects.get(first, SectState(first, first)).world
    ...
    news.extend(deps._advance_wars_unit(game, rng))

    allied_race = any(
        relation.get("status") in {"alliance", "vassal"}
        and deps._player_allegiance_race(game.player) in split_race_pair(key)
        and all(
            game.player.world in RACE_DEFINITIONS.get(side, {}).get("worlds", [])
    ...
    if deps._world_supports(game.player.world, "races") and allied_race:
        reward = float(config.get("alliance_opportunity_reward", 2))
        deps._add_opportunity(game.player, reward)
    ...
    if rng.random() < float(config.get("unit_event_chance", 0.08)):
        deps._random_race_diplomacy_event(game, rng, news)
    if rng.random() < float(config.get("unit_event_chance", 0.08)):
        deps._random_sect_diplomacy_event(game, rng, news)
    deps._maybe_npc_found_power(game, rng, news)
    deps._pressure_weak_npc_powers(game, rng, news)
    deps._simulate_cultivator_duel(game, rng, news)
    return news


def _random_race_diplomacy_event(deps: FactionDependencies, game: GameState, rng: random.Random, news: list[str]) -> None:
    world = game.player.world if deps._world_supports(game.player.world, "races") else "spirit"
...
def _simulate_cultivator_duel(deps: FactionDependencies, game: GameState, rng: random.Random, news: list[str]) -> None:
    world = game.player.world
    people = list({npc.id:npc for npc in deps._all_world_npcs(game) if npc.alive and npc.world == world}.values())
    if len(people) < 2:
        return
    ...
    if demonic and rng.random() < 0.72:
        ...
        first_faction = deps._npc_faction_id(game, first.id)
        opponents = [
            npc for npc in people if npc.id != first.id
            and not (first_faction and deps._npc_faction_id(game, npc.id) == first_faction)
        ]
        if not opponents:
            return
        ...
        pairs = [
            ...
            if not (
                ...
                and deps._npc_faction_id(game, first.id)
                and deps._npc_faction_id(game, first.id) == deps._npc_faction_id(game, second.id)
            )
        ]
        if not pairs:
            return
        first, second = rng.choice(pairs)
    first_power, second_power = deps._npc_power(first), deps._npc_power(second)
    ...
    lethal_chance = deps._npc_lethal_chance(world, loser.realm_index, "duel")
    ...
    if lethal:
        ...
        transfer_summary = deps._maybe_transfer_player_dependency(
            game, loser, winner, rng, context="cultivator_duel",
        )
    faction_ids = {deps._npc_faction_id(game, first.id), deps._npc_faction_id(game, second.id)} - {None}
    ...
    summary = (
        f"{first.name}（{deps._npc_realm_name(first)}）与{second.name}（{deps._npc_realm_name(second)}）"
...
def _set_diplomatic_relation(
    ...
    if status == "truce":
        ...
        power = deps._race_power if kind == "race" else deps._sect_power
...
def _race_power(deps: FactionDependencies, game: GameState, race_id: str) -> float:
    world = game.player.world if deps._world_supports(game.player.world, "races") else "spirit"
    members = [npc for npc in deps._all_world_npcs(game) if npc.alive and npc.world == world and npc.race == race_id]
    return sum(sorted((deps._npc_power(npc) for npc in members), reverse=True)[:5])


def _sect_power(deps: FactionDependencies, game: GameState, sect_id: str) -> float:
    sect = game.sects.get(sect_id)
    return sum(sorted((deps._npc_power(npc) for npc in deps._sect_members(game, sect) if npc.alive), reverse=True)[:5]) if sect else 0.0


def _simulate_war_casualties(deps: FactionDependencies, game: GameState, rng: random.Random, kind: str) -> list[str]:
    ...
    for key, relation in relations.items():
        ...
        for side in (first, second):
            if kind == "race":
                world = game.player.world if deps._world_supports(game.player.world, "races") else "spirit"
                candidates = [npc for npc in deps._all_world_npcs(game) if npc.alive and npc.world == world and npc.race == side]
            ...
            if victim.realm_index >= elite_realm and rng.random() >= deps._npc_lethal_chance(world, victim.realm_index, "war"):
                continue
            victim.alive = False
            victim.death_reason = "势力大战中陨落"
            fallen.append(f"{side_name}{victim.name}（{deps._npc_realm_name(victim)}）")
            for sect in game.sects.values():
                if victim in sect.npcs:
                    deps._check_sect_extinction(game, sect)
        if fallen:
...
def _maybe_race_war_ambush(deps: FactionDependencies, game: GameState, rng: random.Random) -> bool:
    player = game.player
    if not deps._world_supports(player.world, "races") or game.pending_event:
        return False
    enemies = []
    player_race = deps._player_allegiance_race(player)
    for key, relation in game.race_relations.items():
        first, second = split_race_pair(key)
        ...
        if player.world not in RACE_DEFINITIONS.get(enemy, {}).get("worlds", []):
            continue
        if not deps._revenge_ready(game, "race_war", enemy):
            continue
    ...
    target = deps._generate_cultivator_target(player, "边境截杀者", ACTIONS["slay"]["combat"], rng, game=game, forced_race=enemy)
    ...
    interval = deps._record_revenge_trigger(game, "race_war", enemy)
