...
def _party_crossing_candidate(deps: RelationshipDependencies, game: GameState, npc_id: str) -> dict[str, Any] | None:
    player = game.player
...
def _revenge_ready(deps: RelationshipDependencies, game: GameState, family: str, adversary_id: str) -> bool:
    """All revenge sources share one global gate and also retain per-source gates."""
    global_next = int(game.governance_actions.get(deps._revenge_cooldown_key("next"), -1))
    source_next = int(game.governance_actions.get(
        deps._revenge_cooldown_key("next", family, adversary_id), -1,
    ))
    return game.diplomacy_unit >= max(global_next, source_next)


def _record_revenge_trigger(deps: RelationshipDependencies, game: GameState, family: str, adversary_id: str) -> int:
    ...
    global_count_key = deps._revenge_cooldown_key("count")
    source_count_key = deps._revenge_cooldown_key("count", family, adversary_id)
    ...
    game.governance_actions[deps._revenge_cooldown_key("next")] = next_unit
    game.governance_actions[
        deps._revenge_cooldown_key("next", family, adversary_id)
    ] = next_unit
    return interval


def _personal_npcs(deps: RelationshipDependencies, game: GameState) -> list[SectNpc]:
    return list({
        npc.id:npc for npc in deps._all_world_npcs(game)
        if npc.alive and npc.world == game.player.world and npc.id != "player"
    }.values())


def _high_affinity_npcs(deps: RelationshipDependencies, game: GameState, exclude_id: str = "") -> list[SectNpc]:
    threshold = float(WORLD_SYSTEMS["relationship"]["positive_affinity_threshold"])
    people = [npc for npc in deps._personal_npcs(game) if npc.id != exclude_id and float(npc.affinity or 0) >= threshold]
...
def _maybe_affinity_gift(deps: RelationshipDependencies, game: GameState, rng: random.Random) -> bool:
    ...
    event = deps.events_by_id["EVT_PERSONAL_AFFINITY_GIFT_001"]
...
def _maybe_personal_revenge(deps: RelationshipDependencies, game: GameState, rng: random.Random) -> bool:
    ...
    enemies = [
        npc for npc in deps._personal_npcs(game)
        if float(npc.affinity or 0) <= threshold and npc.id not in protected_ids
        and deps._revenge_ready(game, "personal", npc.id)
    ]
    enemies = deps._filter_personal_revenge_by_protection(game, enemies, rng)
    if not enemies:
        return False
    allies = deps._high_affinity_npcs(game)
    ...
    target = {
        "target_name":enemy.name,"target_power":deps._npc_power(enemy),"primary_power":deps._npc_power(enemy),
        ...
        "target_realm_display":deps._npc_realm_name(enemy) if enemy.realm_index <= game.player.realm_index + 1 else "无法看清",
        ...
        "faction_id":deps._npc_faction_id(game,enemy.id),"treasure_item_id":enemy.treasure_item_id,
        "kill_karma":False,"action":"revenge", "player_defending": True,
    }
    event = deps.events_by_id["EVT_PERSONAL_REVENGE_001"]
    game.pending_event = deps._instantiate_event(event, game, rng)
    best_ally = max((npc for npc in allies if npc.id != enemy.id),key=deps._npc_power,default=None)
    sect = game.sects.get(game.player.faction_id or "")
    sect_defenders = [npc for npc in deps._sect_members(game,sect) if npc.alive and npc.id != enemy.id] if sect else []
    game.pending_event["runtime"] = {
        ...
        "sect_support_power":max((deps._npc_power(npc) for npc in sect_defenders),default=0.0),
    ...
    interval = deps._record_revenge_trigger(game, "personal", enemy.id)
...
def _try_conceive_child(deps: RelationshipDependencies, game: GameState, rng: random.Random) -> str:
    ...
    child = {
        ...
        "spirit_root_name":deps._npc_root_name(child_root), "cultivation_started":False,
    ...
    if player.path == "monster" and deps.bloodline_content_available() and rng.random() < float(inheritance.get("species_chance", 1.0)):
...
def _annual_offspring_and_family_update(deps: RelationshipDependencies, game: GameState, rng: random.Random) -> list[str]:
    ...
    for npc in family.npcs:
        ...
        npc.age += 1
        tribulation = deps._resolve_npc_periodic_tribulation(game, npc, rng, family.name)
...
def _relationship_cultivation_perception(
    deps: RelationshipDependencies, game: GameState, person: dict[str, Any], title: str = "故交",
) -> dict[str, Any]:
    ...
    perception = deps._npc_cultivation_perception(game, shell)
    ...
    if perception["display_power"] is not None and person.get("combat_power") is not None:
        actual = max(1.0, deps._npc_power(shell))
...
def _relationship_snapshot(
    deps: RelationshipDependencies, person_id: str, name: str, realm_index: int, layer: int, source: str,
    ...
    return {
        ...
        "realm_name": deps._npc_realm_name(shell), "source": source,
        "age": age, "lifespan": lifespan, "alive": alive, "death_reason": death_reason,
        "spirit_root": spirit_root, "spirit_root_name": deps._npc_root_name(spirit_root),
...
def _annual_relationship_update(deps: RelationshipDependencies, game: GameState, rng: random.Random | None = None) -> None:
    ...
    for relation in event_relations:
        ...
        relation.update(
            ...
            spirit_root=shell.spirit_root, spirit_root_name=deps._npc_root_name(shell.spirit_root),
