...
def _public_family(deps: FactionViewDependencies, game: GameState) -> dict[str, Any]:
    ...
    offspring = [
        {
            **copy.deepcopy(child),
            "gender_name": gender_name(str(child.get("gender") or deps._stable_gender(str(child.get("id", ""))))),
            "spirit_root_name": deps._npc_root_name(str(child.get("spirit_root", "none"))),
            "realm_name": (
                deps._npc_realm_name(SectNpc(
    ...
    for npc in sorted(family.npcs if visible_family else [], key=lambda row:(-row.realm_index,-row.layer,row.name)):
        roster.append({
            **npc.to_dict(), "realm_name":deps._npc_realm_name(npc),
            "spirit_root_name":deps._npc_root_name(npc.spirit_root),
            "combat_power":deps._npc_power(npc) if npc.alive else 0,
...
def _public_governance(deps: FactionViewDependencies, game: GameState) -> dict[str, Any]:
    candidates: dict[str, dict[str, Any]] = {}
    for npc in [*game.world_npcs.values(), *game.notable_npcs.values()]:
        if npc.alive and npc.world == game.player.world:
            candidates[npc.id] = {
                "id":npc.id,"name":npc.name,"realm_name":deps._npc_realm_name(npc),
                "source":"固定人物" if npc.id in game.world_npcs else "留名人物",
                "combat_power":round(deps._npc_power(npc),1),
            }
    for row in game.encounter_npc_cache:
        ...
        if raw.get("alive", True) and raw.get("world") == game.player.world:
            ...
            candidates[npc.id] = {
                "id":npc.id,"name":npc.name,"realm_name":deps._npc_realm_name(npc),
                "source":"路人缓存池","combat_power":round(float(row.get("combat_power",deps._npc_power(npc))),1),
            }
    race_voice = deps._has_race_voice(game)
    sect_voice = deps._has_sect_voice(game)
    family_voice = deps._has_family_voice(game)
...
def _public_faction(deps: FactionViewDependencies, game: GameState) -> dict[str, Any]:
    player = game.player
    ...
    if not player.faction_id:
        return {
            ...
            "can_found": player.alive, "founding_threshold": deps._governance_threshold(player.world),
        }
    if (
        ...
        return {
            ...
            "world_name": world_name, "system_available": bool(available),
            "departed_human_world": player.world != "human",
            "can_found": False, "founding_threshold": deps._governance_threshold(player.world),
        }
    sect = game.sects[player.faction_id]
    definition = deps._faction_meta(game, sect.id)
    unlocked = player.realm_index >= 4
    sect_members = deps._sect_members(game, sect)
    roster = []
    for npc in sect_members:
        ...
        roster.append({
            **public_npc,
            "title": deps._dynamic_sect_title(npc, sect),
    ...
    sect_hostility = player.hostility.get(deps._hostility_key("sect", sect.id), 0)
    ...
    roster.append({
        ...
        "breakthrough_chance": deps._breakthrough_chance(player, deps._manual_breakthrough_kind(player) == "major")["final"] if deps._manual_breakthrough_kind(player) else None,
        "treasure_name": None,
        "in_party": False,
    ...
    return {
        ...
        "succession_plan": copy.deepcopy(
            deps._intrigue_state(game).get("succession_plans", {}).get(sect.id)
        ),
        "pressure": sect.pressure,
        "pressure_limit": int(WORLD_SYSTEMS["player_faction"]["pressure_limit"]),
        "has_diplomatic_voice": deps._has_sect_voice(game),
        "diplomacy": deps._public_sect_diplomacy(game, sect),
        "diplomacy_events": [
...
def _public_sect_diplomacy(deps: FactionViewDependencies, game: GameState, sect: SectState) -> list[dict[str, Any]]:
    ...
    for other in game.sects.values():
        ...
        living.sort(key=lambda npc: (-deps._npc_power(npc), -npc.realm_index, -npc.layer))
        recent = [
        ...
        rows.append({
            ...
            "combined_power": round(sum(deps._npc_power(npc) for npc in living), 1),
            "leaders": [f"{npc.name}（{deps._npc_realm_name(npc)}）" for npc in living[:3]],
            "recent_events": recent,
            "transfer_candidates": deps._vassal_transfer_candidates(game, "sect", sect.id, other.id, relation),
        })
    return rows


def _vassal_transfer_candidates(
    deps: FactionViewDependencies, game: GameState, kind: str, own_id: str, target_id: str, relation: dict[str, Any],
) -> list[dict[str, Any]]:
    if relation.get("status") != "vassal" or relation.get("overlord") != own_id or relation.get("subject") != target_id:
        return []
    player_rank = deps._actual_player_realm(game.player)
    if kind == "sect":
