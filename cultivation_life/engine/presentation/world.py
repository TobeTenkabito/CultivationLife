...
from ..dependencies import WorldViewDependencies
        ...
        result.append({
            **public_npc,
            ...
            "combat_power": (
                ...
                * deps._npc_formation_power_multiplier(game, npc.id)
            ...
            "formation": (
                {
                    ...
                    "bonus": round((deps._npc_formation_power_multiplier(game, npc.id) - 1.0) * 100.0, 2),
                }
                if same_world and npc.alive and npc.id in game.npc_formations else None
            ),
            "attitude": attitude_label(
                npc.affinity or 0,
                game.player.hostility.get(deps._hostility_key("race", npc.race), 0),
...
def _public_spirit_ranking(deps: WorldViewDependencies, game: GameState) -> dict[str, Any]:
    ...
    for npc in npcs:
        ...
        rows.append({
            ...
            "realm_name": deps._npc_realm_name(npc), "race": npc.race,
            "race_name": RACE_DEFINITIONS.get(npc.race, {"name": npc.race})["name"],
    ...
    if companion and companion.get("alive", True) and companion.get("world") == ranking_world and companion.get("id") not in seen_ids:
        rows.append({
            ...
            "combat_power": deps._relationship_combat_power(companion), "is_player": False,
        })
    ...
    for friend in game.player.dao_friends:
        ...
        rows.append({
            ...
            "realm_name": deps._npc_realm_name(shell), "race": friend.get("race", "human"),
            "race_name": RACE_DEFINITIONS.get(friend.get("race", "human"), {"name": "种族未明"})["name"],
            "combat_power": deps._relationship_combat_power(friend), "is_player": False,
        })
    ...
    rows.append({
        ...
        "realm_name": deps._npc_realm_name(player_shell), "race": game.player.race,
        "race_name": RACE_DEFINITIONS.get(game.player.race, {"name": game.player.race})["name"],
        "combat_power": deps._player_intrinsic_combat_power(game.player), "is_player": True,
    })
...
def _public_race_system(deps: WorldViewDependencies, game: GameState) -> dict[str, Any]:
    race_world = game.player.world
    available = deps._world_supports(race_world, "races")
    player_race = deps._player_allegiance_race(game.player)
    ...
    for race_id, definition in RACE_DEFINITIONS.items():
        ...
        for other_id, other_definition in RACE_DEFINITIONS.items():
            ...
            relations.append({
                ...
                "transfer_candidates": deps._vassal_transfer_candidates(game, "race", race_id, other_id, relation),
            })
        recent_events = [
    ...
    return {
        ...
        "has_diplomatic_voice": deps._has_race_voice(game),
        "vassal_transfers": [
            {"target_id": other_id, "target_name": RACE_DEFINITIONS[other_id]["name"], "candidates": deps._vassal_transfer_candidates(game, "race", player_race, other_id, relation)}
            for key, relation in game.race_relations.items()
            ...
            if (
                ...
                and deps._vassal_transfer_candidates(game, "race", player_race, other_id, relation)
            )
        ],
    }


def _public_world_route(deps: WorldViewDependencies, game: GameState) -> dict[str, Any]:
    player = game.player
    route_id, route = next(
        (
            (route_id, definition)
    ...
    return {
        ...
        "allegiance_race_name": RACE_DEFINITIONS.get(deps._player_allegiance_race(player), {"name":deps._player_allegiance_race(player)})["name"],
        "stages": stages,
    }
