...
from ..content_registry import FACTION_SYSTEMS, REALMS, WORLD_SYSTEMS, STORY_COMBAT_SCENARIOS
...
from ..system.transformation_system import active_transformation_profile
from .engine_constants import OPS
from .dependencies import EventDependencies
...
def _select_event(deps: EventDependencies, game: GameState, action: str, rng: random.Random) -> dict[str, Any] | None:
    ...
    for event in sorted(deps.events, key=lambda value: value["id"]):
        ...
        if "manual_only" in tags:
            continue
        if "revenge" in tags and not deps._revenge_ready(game, "ambient", str(event["id"])):
            continue
        ...
        if event.get("repeat") == "once" and any(h.event_id == event["id"] for h in game.history):
            continue
        weight = deps._event_weight(event, game, action)
    ...
    for event, weight in candidates:
        ...
        if roll <= 0:
            if "revenge" in event.get("tags", []):
                deps._record_revenge_trigger(game, "ambient", str(event["id"]))
            return event
    selected = candidates[-1][0]
    if "revenge" in selected.get("tags", []):
        deps._record_revenge_trigger(game, "ambient", str(selected["id"]))
    return selected
...
def _instantiate_event(deps: EventDependencies, event: dict[str, Any], game: GameState, rng: random.Random) -> dict[str, Any]:
    ...
    for choice in result["choices"]:
        ...
        choice["enabled"] = deps._condition(original.get("conditions", {}), game)
    ...
    if event.get("combat"):
        ...
        if deps._world_supports(game.player.world, "races"):
...
def _condition(deps: EventDependencies, condition: dict[str, Any], game: GameState) -> bool:
    if not condition:
        return True
    if "all" in condition:
        return all(deps._condition(entry, game) for entry in condition["all"])
    if "any" in condition:
        return any(deps._condition(entry, game) for entry in condition["any"])
    if "not" in condition:
        return not deps._condition(condition["not"], game)
    ...
    if "has_affinity" in condition:
        return condition["has_affinity"] in [*deps._base_affinities(game.player), *game.player.additional_roots]
    left = deps._path(condition["path"], game)
    return OPS[condition.get("op", "eq")](left, condition.get("value"))


def _path(deps: EventDependencies, path: str, game: GameState) -> Any:
    mapping = {
...
def _resolve_story_combat_check(
    ...
    for member in scenario["enemy_members"]:
        ...
        members.append({
            ...
            "full_power": deps._story_unit_full_power(member, member_realm, effective_power),
    ...
    deps._die(game, reason, event_id)
    if game.last_combat_report:
