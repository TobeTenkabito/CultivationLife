...
def _resolve_trial_step(deps: TrialDependencies, game: GameState, step: str, rng: random.Random) -> tuple[str, str]:
    player = game.player
    ...
    if kind == "heavenly_demon":
        return deps._resolve_heavenly_demon_battle(game, step, rng)
    if kind == "celestial_ascension":
        return deps._resolve_celestial_ascension_step(game, step, rng)
    if kind == "asura_ascension":
        return deps._resolve_asura_ascension_step(game, step, rng)
    passed = False
    detail = ""
    if kind == "traditional":
        ...
        if step == "heaven_vitality":
            ...
            actual = deps._player_intrinsic_combat_power(player)
    ...
    if not passed:
        ...
        if lethal:
            deps._die(game, f"未能渡过{deps.events_by_id[game.pending_event['id']]['title'] if game.pending_event else '劫难'}", "SYS_TRIAL_FAILED")
            return "dead", f"判定失败（{detail}），心魔 +{gain:g}，你在劫中陨落。"
        return "breakthrough_failed", f"判定失败（{detail}），你保住性命但冲关中止，心魔 +{gain:g}。"

    reduction = deps._tribulation_damage_reduction(player, kind) if kind in {"heavenly", "periodic_thunder"} else 0.0
    damage_multiplier = (
    ...
    if trial["step_index"] < len(event_ids):
        next_event = deps.events_by_id[event_ids[trial["step_index"]]]
        game.pending_event = deps._instantiate_event(next_event, game, rng)
...
def _resolve_celestial_ascension_step(
    deps: TrialDependencies, game: GameState, step: str, rng: random.Random,
) -> tuple[str, str]:
    player = game.player
    trial = game.active_trial
    if not trial:
    ...
    if step == "ascension_body":
        ...
        reduction = deps._tribulation_damage_reduction(player, "celestial_ascension")
    ...
    if not passed:
        ...
        player.next_thunder_damage_reduction = 0.0
        game.active_trial = None
        deps._die(game, f"九重飞升劫的{step}判定失败，肉身与元神一同崩解", "SYS_CELESTIAL_ASCENSION_FAILED")
...
def _resolve_asura_ascension_step(
    ...
    if step == "asura_body":
        ...
        reduction = deps._tribulation_damage_reduction(player, "asura_ascension")
    ...
    if not passed:
        ...
        player.next_thunder_damage_reduction = 0.0
        game.active_trial = None
        deps._die(game, f"九重修罗天魔劫的{step}判定失败，魔躯与元神一同崩解", "SYS_ASURA_ASCENSION_FAILED")
...
def _complete_immortal_conversion_stage(deps: TrialDependencies, game: GameState, expected_stage: int) -> tuple[str, str]:
    player = game.player
...
def _tribulation_damage_reduction(deps: TrialDependencies, player: Player, kind: str = "heavenly") -> float:
    item_reduction = (
...
def _check_tribulation(deps: TrialDependencies, game: GameState, rng: random.Random) -> None:
    player = game.player
    ...
    world_cap = deps._tribulation_base_power_cap(player.world)
