...
from ..content_registry import ITEM_CATALOG, REALMS, RACE_DEFINITIONS, RACE_SYSTEMS, WORLD_SYSTEMS
...
from ..system.crafting_system import crafted_combat_effects
from ..system.formation_system import (
    active_formation_profile,
...
def _combat_battlefield_tags(deps: CombatDependencies, game: GameState, target: dict[str, Any]) -> list[str]:
    ...
    location = deps.maps.location(game.player.world, location_id)
...
def _record_player_combat(
    deps: CombatDependencies, game: GameState, target: dict[str, Any], resolution: Any, result: str,
) -> None:
...
def _combat(deps: CombatDependencies, game: GameState, target: dict[str, Any], lethal: bool, rng: random.Random) -> tuple[str, str]:
    ...
    ground_array = None if portable_formation.get("active") else deps._local_ground_formation(game)
    if ground_array:
        ground_profile = deps._ground_profile(player, ground_array)
    ...
    enemy_formation_id = max(
        npc_ids, key=lambda npc_id: deps._npc_formation_power_multiplier(game, npc_id), default="",
    )
    if enemy_formation_id:
        enemy_entry = game.npc_formations[enemy_formation_id]
        enemy_profile = deps._npc_formation_profile(
    ...
    if player.world == "celestial":
        ...
        if deps._court_law_active(game, "immortal_slaughter"):
            player.karma -= 10
        if (
    ...
    player_units = deps._player_combat_units(game, target)
    ...
    if formation_gain > 0:
        deps._grant_art_experience(player, "formation", formation_gain)
    ...
    if ground_array and resolution.formation_integrity_end is not None:
        ...
        settings = deps._formation_rules()
    ...
    if target.get("combat_type") == "beast":
        ...
        if resolution.outcome == "victory":
            ...
            demonic_gain = deps._grant_demonic_kill_opportunity(
    ...
    if resolution.outcome != "victory":
        ...
        if player.hp <= 0:
            can_take_captive = bool(
                ...
                and deps._post_battle_possession_candidates(game)
            )
            captured = False if can_take_captive else deps._capture_defeated_ghost(game, target, rng)
            if not captured:
                deps._die(
...
def _apply_cultivator_kill(deps: CombatDependencies, game: GameState, victim: dict[str, Any], rng: random.Random) -> None:
    ...
    wartime_opponent = deps._is_wartime_opponent(game, str(faction_id) if faction_id else None, race)
    if faction_id:
        key = deps._hostility_key("sect", str(faction_id))
        if faction_id == player.faction_id:
            deps._handle_same_sect_kill(game, str(faction_id), str(npc_id) if npc_id else None)
        elif not wartime_opponent and deps._kill_generates_hostility(game, "sect", str(faction_id), int(victim["realm_index"])):
    ...
    if (
        deps._world_supports(player.world, "races") and race != deps._player_allegiance_race(player)
        and not wartime_opponent
        and deps._kill_generates_hostility(game, "race", race, int(victim["realm_index"]))
    ):
        key = deps._hostility_key("race", race)
        player.hostility[key] = player.hostility.get(key, 0) + float(config["kill_hostility_gain"])


def _kill_generates_hostility(deps: CombatDependencies, game: GameState, kind: str, target_id: str, victim_realm: int) -> bool:
    ...
    if victim_realm < minimum:
        return False
    if kind == "race":
        relation = game.race_relations.get(race_pair(deps._player_allegiance_race(player), target_id), {})
...
def _is_wartime_opponent(deps: CombatDependencies, game: GameState, faction_id: str | None, race_id: str) -> bool:
    player = game.player
    ...
    for war in game.wars:
        ...
        own_side = deps._participant_side(war, own_id)
        target_side = deps._participant_side(war, target_id)
...
def _handle_same_sect_kill(deps: CombatDependencies, game: GameState, faction_id: str, current_victim_id: str | None = None) -> None:
    player = game.player
    config = WORLD_SYSTEMS["faction_conflict"]
    ...
    hostility_key = deps._hostility_key("sect", faction_id)
    if player.realm_index < control_realm:
        ...
        result = "warned"
        event = deps.events_by_id.get("EVT_SECT_FIRST_WARNING_001")
        if event:
            game.pending_event = deps._instantiate_event(event, game, random.Random(f"warning:{game.seed}:{player.age}"))
    elif not is_first:
        player.faction_id = None
...
def _die(
    deps: CombatDependencies, game: GameState, reason: str, event_id: str, *,
    ...
    if offer_captive_possession:
        deps._prepare_post_battle_possession(game, event_id)
