...
from ..dependencies import InventoryDependencies
...
def use_item(deps: InventoryDependencies, game_id: str, item_id: str) -> dict[str, Any]:
    ...
    if game.pending_event:
        ...
        if game.player.path == "monster" and scope_type == "major" and deps.bloodline_content_available():
        ...
        if affinity in game.player.additional_roots or affinity in deps._base_affinities(game.player):
...
def buy_market_offer(deps: InventoryDependencies, game_id: str, offer_id: str) -> dict[str, Any]:
    game = deps._load(game_id)
    if game.pending_event:
...
def equip_known_technique(deps: InventoryDependencies, game_id: str, technique_id: str, slot: str) -> dict[str, Any]:
    game = deps._load(game_id)
    if game.pending_event:
        raise ValueError("请先处理当前事件")
...
def upgrade_technique(deps: InventoryDependencies, game_id: str, technique_id: str) -> dict[str, Any]:
    game = deps._load(game_id)
...
def merge_technique_manuals(
    deps: InventoryDependencies, game_id: str, technique_id: str, level: int,
) -> dict[str, Any]:
    game = deps._load(game_id)
    if game.pending_event or not game.player.alive or game.player.imprisonment:
...
def absorb_transformation_material(
    deps: InventoryDependencies, game_id: str, item_id: str, purify: bool = False, stat_id: str = "",
) -> dict[str, Any]:
    game = deps._load(game_id)
    player = game.player
...
def batch_absorb_transformation_material(
    deps: InventoryDependencies, game_id: str, item_id: str, mode: str = "direct", stat_id: str = "",
) -> dict[str, Any]:
    game = deps._load(game_id)
    player = game.player
    if player.path == "monster":
...
def manage_transformation(deps: InventoryDependencies, game_id: str, form_id: str, action: str) -> dict[str, Any]:
    game = deps._load(game_id)
    player = game.player
    if player.path == "monster":
